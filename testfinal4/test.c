...
int check_pos(int numb_note, int note, int previous){
        ...
        if (numb_note > 1 && previous == 0) {//if more the 1 note and this is the first time
                ...
                if(note > 19) {
                        ...
                        writeNumb();
                        finger_right(note - temp + 1 + 5); // control rotation of first finger
                        ena();
                        usleep(75000);
                        disable();
...
int* mapping_general3(int numb_note, char* notes){
        char map[5] = "fgabcde";
        char letter[3];
        int index_note = 0;
        int index_list = 0;
        int set_bit = 0;
        static int note_list [5];
        for (int i = 0; i < (strlen(notes) / 5); i+=5) {
                while (notes[index_note] != ' ') {
                        if(notes[index_note + i] == map[index_note] && index_note == 0) {
                                note_list[index_list] = index_note;
                                index_note++;
                                if (index_note < 4) {
                                        set_bit = 1;
                                } else {
                                ...
                                note_list[index_list] = note_list[index_list] + (set_bit * 7);
                        }
                }
                index_note = 0;
        }
